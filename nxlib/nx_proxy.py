...
class NxLibException(Exception):
    def __init__(self, message, path, errorCode):
        ...
        self.message = str(message) + str(' at ') + str(self.path) + \
            str(' - ErrorCode ') + str(self.errorCode)
...
class NxLib(object):
    ...
    def setBinaryFormatted(self, path, buffer, width, height, channelCount, bytesPerElement, isFloat):
        ...
        buffer = cast(buffer, POINTER(c_void_p))
        errorCode = c_int32(0)
        f(byref(errorCode), path, buffer, width, height,
          channelCount, bytesPerElement, isFloat)
        return errorCode.value
    ...
    def getBinary(self, path, destinationBuffer, bufferSize):
        ...
        result = f(byref(errorCode), path, destinationBuffer,
                   bufferSize, byref(bytesCopied), byref(timestamp))
    ...
    def getBinaryInfo(self, path):
        ...
        f(byref(errorCode), path, byref(width), byref(height), byref(channelCount), byref(bytesPerElement),
          byref(isFloat), byref(timestamp))
    ...
    def getJson(self, path, prettyPrint, numberPrecision, scientificNumberFormat):
        ...
        errorCode = c_int32(0)
        result = f(byref(errorCode), path, prettyPrint,
                   numberPrecision, scientificNumberFormat)
    ...
    def getJsonMeta(self, path, numLevels, prettyPrint, numberPrecision, scientificNumberFormat):
        ...
        errorCode = c_int32(0)
        result = f(byref(errorCode), path, numLevels, prettyPrint,
                   numberPrecision, scientificNumberFormat)
        return result.decode(), errorCode.value
...
class NxLibItem(object):
    ...
    def compare(self, value):
        ...
        if type(itemValue) == type(value):
            ...
            raise NxLibException('NxLibException : ',
                                 self.path, NxLibItemTypeNotCompatible)
    ...
    def setBinaryData(self, buffer, bufferSizeOrWidth=0, height=0, channelCount=0, bytesPerElement=0, isFloat=0):
        if bufferSizeOrWidth == 0:
            ...
            if (channelCount > 0):  # formatted
                ...
                errorCode = nxLib.setBinaryFormatted(self.path, buffer, width, height, channelCount, bytesPerElement,
                                                     isFloat)
            else:  # not formatted
    ...
    def setBinaryDataFromCvMat(self, mat):
        ...
        errorCode = nxLib.setBinaryFormatted(
            self.path, buffer, mat.shape[1], mat.shape[0], channelCount, bpe, isFloat)
        self.checkReturnCode(errorCode)
    ...
    def getBinaryData(self, bufferSize=0):
        ...
        bytesCopied, timestamp, errorCode = nxLib.getBinary(
            self.path, cbuffer, bufferSize)
        self.checkReturnCode(errorCode)
        return buffer
    ...
    def createCvMatBuffer(self):
        ...
        image_buffer = np.zeros(
            (height, width, channelCount), nptype, order='C')
    ...
    def getBinaryDataInfo(self):
        width, height, channelCount, bpe, isFloat, timestamp, errorCode = nxLib.getBinaryInfo(
            self.path)
        self.checkReturnCode(errorCode)
    ...
    def asJson(self, prettyPrint=1, numberPrecision=2, scientificNumberFormat=0):
        json, errorCode = nxLib.getJson(
            self.path, prettyPrint, numberPrecision, scientificNumberFormat)
        self.checkReturnCode(errorCode)
        return json

    def asJsonMeta(self, numLevels=1, prettyPrint=1, numberPrecision=2, scientificNumberFormat=0):
        jsonMeta, errorCode = nxLib.getJsonMeta(self.path, numLevels, prettyPrint, numberPrecision,
    ...
    def waitForValue(self, value, waitForEqual):
        if type(value) is int:
            ...
            errorCode = nxLib.waitForStringValue(
                self.path, value, waitForEqual)
        elif type(value) is bool:
